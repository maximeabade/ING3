...
// Fonction qui permet de décrypter une chaine de caractère avec la méthode de codage de Vigenère
void vigenere_decrypt(char *str_chaine)
{
    // Déclaration des variables
    char* str_cle; // la clé de codage
    size_t longueur; // taille de la clé
    ...
    while(str_chaine[int_i] != '\0'){
        ...
        if(isalpha(str_chaine[int_i])) {
            // On traite le cas des lettres majuscules
            if(isupper(str_chaine[int_i])) {
                str_chaine[int_i] = ( str_chaine[int_i] - 65 - int_decalage + 26 ) % 26 + 65;
            }
            // On traite le cas des minuscules
            if(islower(str_chaine[int_i])) {
                str_chaine[int_i] = ( str_chaine[int_i] - 97 - int_decalage + 26 ) % 26 + 97;
            }
            int_j ++;
        }
        int_i ++;
    ...
    // On affiche la chaine de caractère décodée
    printf("\nLa chaine de caractère décodée est : %s\n", str_chaine);
}
...
void scytale_decrypt(char *str_chaine){
    // Déclaration des variables
    char** ppchar_tab; // le tableau de caractères
    int int_i; // variable de boucle
    int int_j; // variable de boucle
    ...
    for(int_i = 0; int_i < int_taille; int_i++) {
        for(int_j = 0; int_j < int_taille; int_j++) {
            if(int_j * int_taille + int_i < strlen(str_chaine) && str_chaine[int_j * int_taille + int_i] != '\n'){
                ppchar_tab[int_i][int_j] = str_chaine[int_j * int_taille + int_i];
            } else {
                ppchar_tab[int_i][int_j] = ' ';
            }
        }
    ...
    for(int_i = 0; int_i < int_taille; int_i++) {
        for(int_j = 0; int_j < int_taille; int_j++) {
            str_chaine[int_i * int_taille + int_j] = ppchar_tab[int_i][int_j];
        }
    }
    str_chaine[int_taille * int_taille] = '\0';
